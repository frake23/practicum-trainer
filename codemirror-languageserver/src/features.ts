...
export class HoverFeature extends DefaultCodeMirrorFeature {
  ...
  public initialize(capabilities: ServerCapabilities<any>): void {
    ...
    const extension = hoverTooltip(async (view, offset) => {
      ...
      const result = await this._client.makeRequest<HoverParams, Hover | null>(
        this._method,
        params
      );
    ...
    const extension = autocompletion({
      override: [
        async (context) => {
          ...
          const result = await this._client.makeRequest<
            CompletionParams,
          ...
          let options = items.map(
            ({
              ...
              const completion: Completion & {
                ...
                type:
                  kind && mapFromEnum(CompletionItemKind)[kind].toLowerCase(),
